...
with b5: st.markdown(f"

*Seçili Bitiş Tarihine göre hesaplar.
...
badge = f"CANLI: {benzin_yeni_val} TL" if yakit_guncel.get("benzin", 0) > 0 else ""

st.markdown(f"

*⛽ Benzin* {badge}", unsafe_allow_html=True)

etiket_b = "Eski (TL) ✅ TCMB" if not is_proxy else "Eski (TL) ⚠️ Tahmin"

st.markdown(f"{etiket_b}", unsafe_allow_html=True)

b_eski = st.number_input("bo", value=benzin_eski_val, key=f"bo_{d_key}", label_visibility="collapsed")

st.markdown("Yeni (TL)", unsafe_allow_html=True)
...
badge_m = f"CANLI: {motorin_yeni_val} TL" if yakit_guncel.get("motorin", 0) > 0 else ""

st.markdown(f"

*🚛 Motorin* {badge_m}", unsafe_allow_html=True)

etiket_m = "Eski (TL) ✅ TCMB" if not is_proxy else "Eski (TL) ⚠️ Tahmin"

st.markdown(f"{etiket_m}", unsafe_allow_html=True)
...
*Hesaplama: Girilen {tr_fmt(sozlesme_tutari)} TL'nin, başlangıç tarihi ve bugünkü kurlar üzerinden karşılığıdır.
...
with c_p2: st.markdown("

*Tahminler bileşik faiz etkisiyle hesaplanır.
